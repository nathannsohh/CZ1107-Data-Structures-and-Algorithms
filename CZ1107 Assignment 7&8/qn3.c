...
void sumToC(LinkedList* ll, int C, ArrayList* al)
{
   ...
   if(ll->sum > C) return;
   if(ll->sum == C){
	   // add the array into the arraylist
	   int *array = malloc(ll->size * sizeof(int));
   ...
   for(int i = 1; i <= C; i++){
	    if(ll->head == NULL) insertNode(ll, 0 , i);
		else{
			if(ll->head->item >= i) continue;
			else insertNode(ll,0,i);
		}
	   sumToC(ll, C, al);
	   removeNode(ll, 0);
   }
