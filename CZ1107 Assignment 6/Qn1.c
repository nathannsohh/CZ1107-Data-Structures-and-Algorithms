...
int HashInsert(HashTable* Q1Ptr, int key)
{
 ...
 int n = Hash(key, Q1Ptr->hSize);
 ListNode *newNode;

 if(HashSearch(*Q1Ptr, key) != NULL) return 0;

 if(Q1Ptr->Table[n].head == NULL){
    Q1Ptr->Table[n].head = malloc((sizeof(ListNode)));
    Q1Ptr->Table[n].head->key = key;
    Q1Ptr->Table[n].head->next = NULL;
    Q1Ptr->Table[n].head->pre = NULL;
    return 1;
 }

 else{
    ListNode *cur;
    cur = Q1Ptr->Table[n].head;
...
{
    //Write your code here
    ListNode *cur = HashSearch(*Q1Ptr,key), *pre, *aft;
    int n = Hash(key, Q1Ptr->hSize);
    if(cur == NULL)
        return 0;
    ...
    if(pre == NULL && aft == NULL){
        Q1Ptr->Table[n].head = NULL;
        free(cur);
        return 1;
    } 
    else if(pre == NULL && aft != NULL){
        Q1Ptr->Table[n].head = aft;
        aft->pre = NULL;
        free(cur);
        return 1;
    }
