...
ListNode* HashSearch(HashTable Q3Hash, int key)
{
 ...
 int n = Hash(key, Q3Hash.hSize);
 ListNode *cur = Q3Hash.Table[n].head;
 while(cur != NULL){
     if(cur->key == key)
        return cur;
    cur = cur->next;
...
{
 // Write your code here
 int n = Hash(key, Q3HashPtr->hSize);
 ListNode *newNode;

 if(Q3HashPtr->Table[n].head == NULL){
    Q3HashPtr->Table[n].head = malloc((sizeof(ListNode)));
    Q3HashPtr->Table[n].head->key = key;
    Q3HashPtr->Table[n].head->next = NULL;
    return 1;
 }

 else{
    ListNode *cur;
    cur = Q3HashPtr->Table[n].head;
